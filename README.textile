h1. Harvesting _OJS_ Journals and _OMP_ Books

h2. Was macht _ulb-journal-saf_


Das Script _journal2saf.py_ ruft von einem konfigurierbaren Server alle relevanten Daten von bereits pulizierten Journalen/Büchern ab.
Das Python-Script nutzt dazu das API des Monograph~/Journalservers.
Relevante Daten sind hier die zentrale PDF Datei und die Metadaten zum Buch bzw. Journal.
Die aggregierten Daten werden in SAF Archivdateien pro Buch~ bzw. Journalausgabe gespeichert.
*"SAF":(https://wiki.lyrasis.org/display/DSDOC5x/Importing+and+Exporting+Items+via+Simple+Archive+Format) Archive werden vom Dokumentenserver DSpace als Importformat akzeptiert.*
Die Übertragung zum Dspace Server und der Import der SAF Archive sind im Projekt integriert.
Alternativ können die Dateien auch in einem Austauschordner abgelegt werden, der für Dspace erreichbar ist.
Per cronjob importiert DSpace die Daten und hintelegt eine aktuelle (handle:doi)-Paar Liste.
Diese (handle:doi)-Paar Liste nutzt py2saf, um den DOI im OJS/OMP einzutragen.
...
Um das Datenbankfeld pub-id::doi anzulegen, ist es notwendig das DOI Plugin zu aktivieren!
